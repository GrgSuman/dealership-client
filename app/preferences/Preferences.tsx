...
import { Preference, PreferenceCategory } from "@/types/types";
import {
  CAR_TYPES,
  FUEL_TYPES,
  BRANDS,
...
import { addPreference } from "../actions/user/user";
...
export default function Preferences({
  ...
  const [preferences, setPreferences] = useState<Preference>({
    budgetMin: userPreference?.budgetMin || 20000,
    budgetMax: userPreference?.budgetMax || 80000,
    carTypes: userPreference?.carTypes || [],
    fuelTypes: userPreference?.fuelTypes || [],
    brand: userPreference?.brand || [],
    features: userPreference?.features || [],
    primarilyUse: userPreference?.primarilyUse || [],
    topPriorities: userPreference?.topPriorities || [],
  });
  ...
  const handleSavePreferences = async () => {
    setIsLoading(true);
    const newPreference = await addPreference(preferences);
    if (newPreference) {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 text-base">
              Select your preferences to get personalized car recommendations
            </p>
          </div>
          <Button
          ...
          >
            {isLoading ? (
              <div className="flex items-center gap-2">
                ...
                <span>Setting New Preferences...</span>
              </div>
            ) : (
              <div className="flex items-center gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              {BRANDS.map((brand) => (
                <motion.div key={brand} whileTap={{ scale: 0.98 }}>
                  <Badge
                    variant={
                      preferences.brand.includes(brand) ? "default" : "outline"
                    }
                    className={`cursor-pointer text-sm px-3 py-1.5 ${
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-base">
              Your car preferences have been saved successfully. Would you like
              to see recommended vehicles based on your preferences?
            </AlertDialogDescription>
          </AlertDialogHeader>
